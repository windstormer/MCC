...
class Engine:
    ...
    def step(self, data_batch, label_batch, chosen_idx):   
        logit = self.model(data_batch.cuda(), data_batch.shape[0])
    ...
    def inference(self, data_batch, chosen_id_shift):   
        logit = self.model(data_batch.cuda(), data_batch.shape[0])
        pred = torch.sigmoid(logit)
        if chosen_id_shift is None:
            pred_sel = pred
        else:
            pred_sel = torch.index_select(pred, 0, chosen_id_shift.cuda())
        pred_sel = pred_sel.detach().cpu()
        out = torch.where(pred_sel > 0.5, 1, 0)
        return pred_sel, out
    ...
    def ESV_EDV_test(self, save_dir, load_model_path):
        ...
        result_HD95 = []
        with torch.no_grad():
            for (data, (large_label, small_label, large_index, small_index), start_id, video_id) in test_bar:
                data = data.squeeze(0)
                ...
                chosen_id_shift = chosen_id - torch.ones_like(chosen_id)*start_id
                _, out_batch = self.inference(data, chosen_id_shift)
                data = torch.index_select(data, 0, chosen_id_shift)
                
                draw_image_index(test_image_path, video_id[0], data, label, out_batch, chosen_id)
                results = self.evaluate(label.numpy(), out_batch[:2].numpy())
                result_dice.append(results['Dice'])
                result_iou.append(results['IoU'])
    ...
    def test(self, save_dir, load_model_path):
        ...
        with torch.no_grad():
            for image, label, case_id in test_bar:
                ...
                _, out_batch = self.inference(image, None)
