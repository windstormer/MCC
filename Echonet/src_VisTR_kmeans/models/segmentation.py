...
class VisTRsegm(nn.Module):
    ...
    def forward(self, samples, length):
        ...
        bs_f = bs//length
        src_proj = src_proj.reshape(bs_f, length, c, s_h, s_w).permute(0,2,1,3,4).flatten(-2)
        # print("src_proj.shape", src_proj.shape)

        # mask = mask.reshape(bs_f, length, s_h*s_w)
        pos = pos[-1].permute(0,2,1,3,4).flatten(-2)
        hs, memory = self.vistr.transformer(src_proj, self.vistr.query_embed.weight[:length], pos)
        ...
        for i in range(3):
            _,c_f,h,w = features[i].shape
            features[i] = features[i].reshape(bs_f, length, c_f, h,w)
        n_f = 1
        outputs_seg_masks = []
        ...
        for i in range(length):
            ...
            bbox_mask_f = self.bbox_attention(hs_f, memory_f)
            seg_masks_f = self.mask_head(memory_f, bbox_mask_f, [features[2][:,i], features[1][:,i], features[0][:,i]])
            # print(i, seg_masks_f.shape)
        ...
        # print("outputs_seg_masks.shape", outputs_seg_masks.shape)
        outputs_seg_masks = outputs_seg_masks.reshape(1,length,outputs_seg_masks.size(-2),outputs_seg_masks.size(-1))
        out = F.interpolate(outputs_seg_masks, size=(ori_x, ori_y), mode="bilinear")
